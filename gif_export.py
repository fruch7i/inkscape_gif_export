...
class GifExport(inkex.Effect):
    def add_arguments(self, pars):
        pars.add_argument("--folder", default=os.path.expanduser("~"),
            help="Folder name")
        pars.add_argument("--name", type=str, default="inkscape_export.gif",
            help="GIF name")
        pars.add_argument("--reverse", type=inkex.Boolean, default=False,
            help="Reverse order of inkscape objects")
        pars.add_argument("--temp", type=inkex.Boolean, default=False,
            help="Save all frames as png")
        pars.add_argument("--loop", type=int, default=0,
            help="Number of loops, use 0 for infinite")
        pars.add_argument("--duration", type=int, default=100,
            help="Duration in ms")
    ...
    def effect(self):
        ...
        for i, obj_id in enumerate(self.options.ids):
            if self.options.temp:
                png_file = self.options.folder + '/frame_' + str(i) + '.png'
            else:
                png_fd, png_file = tempfile.mkstemp(suffix='.png')
            subprocess.run(['inkscape', '--vacuum-defs', f'--export-id={obj_id}', '--export-id-only', f'--export-filename={png_file}', svg_tmpfile])
            frames.append(IM.open(png_file))
        ...
        if self.options.reverse:
            frames.reverse()
        inkex.errormsg(frames[0].info)
        frames[0] = frames[0].convert(mode='P')
        save_transparent_gif(frames, self.options.duration, filename)
        #frames[0].save(filename, append_images=frames[1:], save_all=True, optimize=True, transparency=0, disposal=2, duration=self.options.duration, loop=self.options.loop)
        inkex.errormsg(f'GIF exported to: {filename}')
